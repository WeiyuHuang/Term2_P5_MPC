...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // 1.1 Minimize cte[t]^2 + epsi[t]^2 + (v[t] - v_ref)^2
    for (int t = 0; t < N; t++) {
    ...
    // 1.2 Minimize delta[t]^2 + a[t]^2 + (delta[t]^2 * a[t]^2)
    for (int t = 0; t < N - 1; t++) {
    ...
    // 1.3 Minimize (delta[t+1] - delta[t])^2 + (a[t+1] - a[t])^2
    for (int t = 0; t < N - 2; t++) {
    ...
    // 2.1 The rest of the constraints
    for (int t = 1; t < N; t++) {
